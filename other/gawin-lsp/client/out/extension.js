...
function activate(context) {
    ...
    client = new node_1.LanguageClient('gawin-lsp', 'Gawin Language Server', serverOptions, clientOptions);
    client.onDidChangeState((event) => {
        outputChannel.appendLine(`[gawin] client state: ${event.oldState} -> ${event.newState}`);
    });
    client.start().then(() => outputChannel.appendLine('[gawin] language client started successfully'), (error) => {
